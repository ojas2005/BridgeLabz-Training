...
namespace AddressBookApp.Services
{
    public class ContactService : IAddressBookService
    {
        ...
        private void ContactServiceDirectoryMenu(ContactDirectory current)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    Console.WriteLine("press 10 to save/load contacts from data source");
        ...
        private void DataSourceMenu(ContactDirectory current)
        {
            try
            {
                Console.WriteLine("press 1 to save contacts to text file (UC 13)");
                Console.WriteLine("press 2 to load contacts from text file (UC 13)");
                Console.WriteLine("press 3 to save contacts to CSV file (UC 14)");
                Console.WriteLine("press 4 to load contacts from CSV file (UC 14)");
                Console.WriteLine("press 5 to save contacts to JSON file (UC 15)");
                Console.WriteLine("press 6 to load contacts from JSON file (UC 15)");
                Console.WriteLine("press 7 to save contacts to JSON server (UC 16)");
                Console.WriteLine("press 8 to load contacts from JSON server (UC 16)");
                Console.WriteLine("press 9 to save contacts to MSSQL database (UC 18)");
                Console.WriteLine("press 10 to load contacts from MSSQL database (UC 18)");
                ...
                switch (choice)
                {
                    case 1:
                        ...
                        SaveContactsAsync(current, new TextFileIO(), txtFilename);
                        break;
                    case 2:
                        Console.WriteLine("enter filename:");
                        string txtLoadFile = Console.ReadLine();
                        LoadContactsAsync(current, new TextFileIO(), txtLoadFile);
                        break;
                    case 3:
                        Console.WriteLine("enter filename:");
                        string csvFilename = Console.ReadLine();
                        SaveContactsAsync(current, new CsvFileIO(), csvFilename);
                        break;
                    case 4:
                        Console.WriteLine("enter filename:");
                        string csvLoadFile = Console.ReadLine();
                        LoadContactsAsync(current, new CsvFileIO(), csvLoadFile);
                        break;
                    case 5:
                        Console.WriteLine("enter filename:");
                        string jsonFilename = Console.ReadLine();
                        SaveContactsAsync(current, new JsonFileIO(), jsonFilename);
                        break;
                    case 6:
                        Console.WriteLine("enter filename:");
                        string jsonLoadFile = Console.ReadLine();
                        LoadContactsAsync(current, new JsonFileIO(), jsonLoadFile);
                        break;
                    case 7:
                        Console.WriteLine("enter server URL (default: http://localhost:3000):");
                        string serverUrl = Console.ReadLine();
                        if (string.IsNullOrWhiteSpace(serverUrl))
                            serverUrl = "http://localhost:3000";
                        Console.WriteLine("enter resource name (e.g., 'contacts'):");
                        string resourceName = Console.ReadLine();
                        SaveContactsAsync(current, new JsonServerIO(serverUrl), resourceName);
                        break;
                    case 8:
                        Console.WriteLine("enter server URL (default: http://localhost:3000):");
                        string serverUrl2 = Console.ReadLine();
                        if (string.IsNullOrWhiteSpace(serverUrl2))
                            serverUrl2 = "http://localhost:3000";
                        Console.WriteLine("enter resource name (e.g., 'contacts'):");
                        string resourceName2 = Console.ReadLine();
                        LoadContactsAsync(current, new JsonServerIO(serverUrl2), resourceName2);
                        break;
                    case 9:
                        
                        string connectionString = "Server=localhost,1433;Database=AddressBookDB;User Id=sa;Password=Insane@1234";
                        SaveContactsToDatabase(current, connectionString);
                        break;
                    case 10:
    
                        string connectionString2 = "Server=localhost,1433;Database=AddressBookDB;User Id=sa;Password=Insane@1234";
                        LoadContactsFromDatabase(current, connectionString2);
                        break;
                    case 11:
                        return;
                    default:
        ...
        private void SaveContactsAsync(ContactDirectory current, IFileIO fileIO, string filename)
        {
            try
            {
                var contacts = current.GetAllContacts();
                
                Thread saveThread = new Thread(() =>
                {
                    try
                    {
                        fileIO.SaveContacts(contacts, filename);
                        Console.WriteLine("save operation completed successfully (UC 17 - non-blocking)");
                    }
                    catch (Exception ex)
                    {
                ...
                saveThread.Start();
                Console.WriteLine("save operation started in background (non-blocking main thread)");
            }
            catch(Exception ex)
            {
                Console.WriteLine($"error initiating save: {ex.Message}");
            }
        }

        private void LoadContactsAsync(ContactDirectory current, IFileIO fileIO, string filename)
        {
            try
            {
                Thread loadThread = new Thread(() =>
                {
                    try
                    {
                        var loadedContacts = fileIO.LoadContacts(filename);
                        ...
                        lock (_lock)
                        {
                            foreach (var contact in loadedContacts)
                            {
                                try
                                {
                                    current.InsertContact(contact);
                                }
                                catch
                                {
                                    Console.WriteLine($"skipped duplicate or invalid contact: {contact.FirstName} {contact.LastName}");
                                }
                            }
                        }
                        
                        Console.WriteLine($"load operation completed successfully (UC 17 - non-blocking). Loaded {loadedContacts.Count} contacts");
                    }
                    catch (Exception ex)
                    {
                ...
                loadThread.Start();
                Console.WriteLine("load operation started in background (non-blocking main thread)");
            }
            catch(Exception ex)
            {
                Console.WriteLine($"error initiating load: {ex.Message}");
            }
        }

        private void SaveContactsToDatabase(ContactDirectory current, string connectionString)
        {
            try
            {
                var contacts = current.GetAllContacts();
                var databaseIO = new MssqlDatabaseIO(connectionString);
                
                Thread saveThread = new Thread(() =>
                {
                    try
                    {
                        databaseIO.SaveContacts(contacts, connectionString);
                        Console.WriteLine("save to MSSQL database completed successfully (UC 18 - non-blocking)");
                    }
                    catch (Exception ex)
                    {
                ...
                saveThread.Start();
                Console.WriteLine("save to MSSQL database started in background (non-blocking main thread)");
            }
            catch(Exception ex)
            {
                Console.WriteLine($"error initiating database save: {ex.Message}");
            }
        }

        private void LoadContactsFromDatabase(ContactDirectory current, string connectionString)
        {
            try
            {
                Thread loadThread = new Thread(() =>
                {
                    ...
                    {
                        var databaseIO = new MssqlDatabaseIO(connectionString);
                        var loadedContacts = databaseIO.LoadContacts(connectionString);
                        ...
                        Console.WriteLine($"load from MSSQL database completed successfully (UC 18 - non-blocking). Loaded {loadedContacts.Count} contacts");
                    }
                    catch (Exception ex)
                    {
                ...
                loadThread.Start();
                Console.WriteLine("load from MSSQL database started in background (non-blocking main thread)");
            }
            catch(Exception ex)
            {
                Console.WriteLine($"error initiating database load: {ex.Message}");
            }
        }
    }
}
